  ...
  return (
    <Portal>
      <Dialog visible={visible} onDismiss={onClose}>
        ...
        <Dialog.Content>
          <TextInput
            label={initialTask ? initialTask.title : "Title"}
            value={formData.title}
            onChangeText={(text) => handleChange('title', text)}
            disabled={initialTask?true:false}
          />
          <TextInput
            label={initialTask ? initialTask.description :"Description"}
          ...
          />
          <TextInput
            label={initialTask ? initialTask.team : "Team"}
            value={formData.team}
            onChangeText={(text) => handleChange('team', text)}
            disabled={initialTask?true:false}
          />
          <TextInput
            label={initialTask? initialTask.assignee : "Assignee"}
       ...
       <View style={{flexDirection:initialTask ? "row":"column", justifyContent: 'space-between',alignItems:initialTask?"center":"flex-start",marginVertical:10}}>
        ...
        <Dialog.Actions>
          {!initialTask && (
            <Button onPress={handleReset} mode="contained">Reset</Button>
          )}
